            ...
            <span>
              ...
              {currencyKeys.length > 1 && (
                <span className="text-xs text-slate-400 font-normal ml-2 block sm:inline">
                  ({currencyKeys.map(code => `${formatCurrency(currencyGroupedBalances[code].total, code)} ${code}`).join(' • ')})
                </span>
              )}
            </span>
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredAccounts.map((account) => {
              ...
              return (
                <div
                  key={account.id}
                  className="growy-glass growy-card-hover rounded-2xl p-6 border border-white/10 relative overflow-hidden flex flex-col justify-between aspect-[1.58/1] min-h-[200px] shadow-xl group transition-all duration-300"
                  style={{ background: bgGradient }}
                >
                  {/* Top Bar: Bank Brand & EMV Chip */}
                  <div className="flex items-start justify-between relative z-10">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center text-xl border border-white/10 shadow-inner">
                        {account.emoji || '💳'}
                      </div>
                      <div>
                        <h3 className="text-base font-bold text-white tracking-tight truncate max-w-[140px]">
                          {account.name}
                        </h3>
                        <span className="text-xs font-semibold text-slate-300 tracking-widest uppercase block">
                    ...
                    {/* Simulation EMV Chip */}
                    <div className="w-9 h-7 rounded-md bg-amber-400/20 border border-amber-300/30 flex items-center justify-center opacity-80 shrink-0">
                      <div className="w-6 h-4 border border-amber-300/40 rounded-sm grid grid-cols-2 gap-0.5 p-0.5">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/10 relative z-10">
                    ...
                    <div className="flex items-center gap-1.5 opacity-90 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-xl text-slate-300 hover:text-rose-400 hover:bg-rose-500/10 transition-colors"
